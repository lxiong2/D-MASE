function H = createHmatrix_rect(e,f,G_a,B_a,type,indices,numbus,buses,buses_a,adjbuses,lines)
...
for a = 1:size(type,1)
    ...
    if strcmp(type(a),'p') == 1
        numPmeas = numPmeas + 1;
        indPmeas = indices(a,:);
        [dPde, dPdf] = realPowerInjMeas_rect(e,f,G_a,B_a,numbus,buses,buses_a,adjbuses,indPmeas);
        H(a,:) = [dPde dPdf];
    % Reactive power injection measurements [dQdth dQdV]   
    elseif strcmp(type(a),'q') == 1
        numQmeas = numQmeas + 1;
        indQmeas = indices(a,:);
        [dQde, dQdf] = reactivePowerInjMeas_rect(e,f,G_a,B_a,numbus,buses,buses_a,adjbuses,indQmeas);
        H(a,:) = [dQde dQdf];
    % Real power flow measurements [dPijdth dPijdV]
    elseif strcmp(type(a),'pf') == 1
        numPFmeas = numPFmeas + 1;
        indPFmeas = indices(a,:);
        [dPijde, dPijdf] = realPowerFlowMeas_rect(e,f,G_a,B_a,numbus,buses_a,lines,indPFmeas);
        H(a,:) = [dPijde dPijdf];
    % Reactive power flow measurements [dQijdth dQijdV]   
    elseif strcmp(type(a),'qf') == 1
        numQFmeas = numQFmeas + 1;
        indQFmeas = indices(a,:);
        [dQijde, dQijdf] = reactivePowerFlowMeas_rect(e,f,G_a,B_a,numbus,buses_a,lines,indQFmeas);
        H(a,:) = [dQijde dQijdf];
