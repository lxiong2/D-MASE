...
while r_largest > badthres
    ...
    if r_largest > badthres %if r_largest > c
        badMeas = [badMeas; indices(ind_largest,:) ind_largest]
        [newz,~] = removerows(z,'ind',ind_largest);
        [newindices,~] = removerows(indices,'ind',ind_largest);
        [newtype,~] = removerows(type,'ind',ind_largest);
        z = newz;
        indices = newindices;
        type = newtype;
        numtype(1) = size(find(strcmp(type,'pf')==1),1);
        numtype(2) = size(find(strcmp(type,'qf')==1),1);
        numtype(3) = size(find(strcmp(type,'p')==1),1);
        numtype(4) = size(find(strcmp(type,'q')==1),1);
        numtype(5) = size(find(strcmp(type,'v')==1),1);
    end
    BDD_iter = BDD_iter + 1;
end
totalt = sum(centralt)
