...
buses3 = [6; 11; 12; 13];
tiebuses3 = [5; 10; 14];
allbuses3 = sort([buses3; tiebuses3]);
numareabus3 = size(buses3,1); % only buses in area
numbus3 = size(allbuses3,1); % includes buses in area + tie buses
...
alltype3(2*numlines3+1:(2*numlines3+numareabus3),:) = repmat({'v'}, [numareabus3 1]);
alltype3(2*numlines3+numareabus3+1:(2*numlines3+3*numareabus3),:)= repmat({'p'; 'q'}, [numareabus3 1]);

allR3 = diag(0.01^2*ones(1,size(alltype3,1)));

% FIX: Need to include those boundary measurements
allindices3 = zeros(2*numlines3+3*numareabus3,3);
for a = 1:numlines3
    allindices3((2*a-1):(2*a),:) = [lines3(a,1:3); lines3(a,1:3)]; 
end
for a = 1:numareabus3
    allindices3(2*numlines3+a,1) = buses3(a);
end
for a = 1:numareabus3
    allindices3(2*numlines3+numareabus3+((2*a-1):(2*a)),1) = buses3(a);
end
...
buses4 = [9; 10; 14];
tiebuses4 = [4; 7; 11; 13];
allbuses4 = sort([buses4; tiebuses4]);
numareabus4 = size(buses4,1); % only buses in area
numbus4 = size(allbuses4,1); % includes buses in area + tie buses
...
alltype4(2*numlines4+1:(2*numlines4+numareabus4),:) = repmat({'v'}, [numareabus4 1]);
alltype4(2*numlines4+numareabus4+1:(2*numlines4+3*numareabus4),:)= repmat({'p'; 'q'}, [numareabus4 1]);

allR4 = diag(0.01^2*ones(1,size(alltype4,1)));

% FIX: Need to include those boundary measurements
allindices4 = zeros(2*numlines4+3*numareabus4,3);
for a = 1:numlines4
    allindices4((2*a-1):(2*a),:) = [lines4(a,1:3); lines4(a,1:3)]; 
end
for a = 1:numareabus4
    allindices4(2*numlines4+a,1) = buses4(a);
end
for a = 1:numareabus4
    allindices4(2*numlines4+numareabus4+((2*a-1):(2*a)),1) = buses4(a);
end
